...
result, sampserror = LearnVanishingIdeal.approximateVanishingIdeal(totalarray, [4]xys, quick=false, affine=false)
@var x,y,z
...
plt = scatter(sample_matrix[1,:], sample_matrix[2,:],
        legend = false, grid = false, framestyle = :origin, markersize=1, markercolor = :grey, ylims = (-5,5), xlims = (-5,5), size=(800,800))
scatter!(plt, orig_sample_matrix[1,:], orig_sample_matrix[2,:],
        markersize=5, markercolor = :steelblue)
